...
//    [self.bodyData appendData:encodeData(kNewLine)];
    ...
    NSString *postLine1 = [NSString stringWithFormat:@"Content-Disposition: form-data; name=\"userfile\"; filename=\"%@\" ", [filePath lastPathComponent]];
    [self.bodyData appendData:encodeData(postLine1)];
    [self.bodyData appendData:encodeData(kNewLine)];
    
    NSString *postLine2 = [NSString stringWithFormat: @"Content-Type: %@", [filePath mimeType]];
    
    [self.bodyData appendData:encodeData(postLine2)];
    [self.bodyData appendData:encodeData(kNewLine)];
    [self.bodyData appendData:encodeData(kNewLine)];
    NSData *fileContent = [[NSData alloc] initWithContentsOfFile:filePath];
    [self.bodyData appendData:fileContent];
}
